...
function getAdminPackageStatus(){
        ...
        if(!empty($package) || app('commonRepo')->adminPackageRepo()->varifyOverdue(admin())){
            $status = '购买';
            $package_id = optional(app('commonRepo')->chargePackageRepo()->model()::where('level',$package->level)->first())->id;
        }
        #当前管理员的店铺
