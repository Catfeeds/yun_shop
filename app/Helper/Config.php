...
//代理商下面的商户id数组集合
function admin_parent_arr($parent_id,$return_arr=true){
    $admins = Admin::where('parent_id',$parent_id)->get();
    if(!$return_arr){
        return $admins;
    }
    $admins_arr = [];
...
function checkLoginPath(){
    ...
    #对应角色跳对应页面
    #管理员
    if($admin->type == '管理员' || $admin->type == '代理商'){
        return redirect($prefix.'/zcjy/package_statics');
    }
    else{
