...
class CommonController extends Controller
{
     ...
     *   tags={"小程序接口:订单支付模块(https://{account}.shop.qijianshen.xyz)"},
     *   summary="小程序获取充值金额列表",
    ...
    /**
     * 小程序确认服务消耗 如果服务次数是多次每次减1 如果只有1次直接消耗
     *
     * @SWG\Get(path="/api/manage/enter_service_consumption",
     *   tags={"小程序[后台管理]接口(https://qijianshen.xyz)"},
     *   summary="确认服务消耗",
     *   description="确认服务消耗接口,管理员业务员都可请求,需要token响应头信息,如果服务次数是多次每次减1 如果只有1次直接消耗",
     *   operationId="enterServiceConsumptionAdmin",
     *   produces={"application/json"},
     *   @SWG\Parameter(
     *     in="query",
     *     name="token",
     ...
     *     @SWG\Response(
     *         response=200,
     *         description="status_code=0请求成功,status_code=1参数错误,data返回消耗服务成功提示",
     *     ),
     *     @SWG\Response(
     *         response=500,
     *         description="服务器出错",
    ...
    public function enterServiceConsumption(Request $request){
        ...
        if($input['account'] == $user->account && $input['account'] ==  $service->account){
            ...
            return zcjy_callback_data('服务使用成功',0);
        }
        else{
            return zcjy_callback_data('account信息错误',1);
        }
    }
