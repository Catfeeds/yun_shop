<table class="table table-responsive" id="managers-table">
    ...
    <tr>
        ...
        <td>@if(!empty($manager->province) && empty($manager->city)) 省级代理 <span class="bitian">({!! app('commonRepo')->cityRepo()->findWithoutFail($manager->province)->name !!})</span> @endif 
            @if(!empty($manager->province) && !empty($manager->city))
            市级代理<span class="bitian">({!! app('commonRepo')->cityRepo()->findWithoutFail($manager->province)->name !!}-{!! app('commonRepo')->cityRepo()->findWithoutFail($manager->city)->name !!})</span>
            @endif
            @if(empty($manager->province) && empty($manager->city))
            普通代理
            @endif
