        ...
        @else
            @if(count($storeShops))
                <div class="hint">
                    ...
                    <a href="javascript:;" data-link="{{ route('package.detail',$admin_package['package_id']).'?type=续费&shop_id='.$i }}">立即续费</a>
                </div>
            @endif
        @endif
                        ...
                        <div>
                            <div class="operate-label">
                                ...
                                {!! Form::open(['route' => ['storeShops.destroy', $shop->id], 'method' => 'delete','style' => 'display:inline']) !!}
                                   {!! Form::button('删除', ['type' => 'submit','class' => 'deleteShop','style'=>'color:#38f;']) !!}
                                {!! Form::close() !!}
                            </div>
                        </div>
